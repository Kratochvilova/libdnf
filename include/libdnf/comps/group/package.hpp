...
class Package {
public:
    explicit Package(std::string name, package_type type, std::string condition) : name(name), type(type), condition(condition) {}

    /// @replaces dnf:dnf/comps.py:attribute:Package.name
    std::string get_name() const { return name; }
    void set_name(const std::string & value) { name = value; }

    package_type get_type() const { return type; }
